'use client'

import React from 'react'
import Header from '@/components/sections/Header'
import Footer from '@/components/sections/Footer'
import { 
  Route, 
  Zap, 
  TrendingUp, 
  CheckCircle, 
  Play,
  ArrowRight,
  Star,
  Clock,
  Globe,
  Smartphone,
  Database,
  BarChart3,
  MapPin,
  Fuel,
  Users,
  Shield
} from 'lucide-react'

export default function RouteOptimizationPage() {
  const features = [
    {
      title: 'AI-Powered Route Planning',
      description: 'Advanced algorithms optimize routes for maximum efficiency',
      icon: <Route className="w-8 h-8" />,
      benefits: ['Multi-stop optimization', 'Traffic integration', 'Weather considerations']
    },
    {
      title: 'Real-time Traffic Updates',
      description: 'Live traffic data for dynamic route adjustments',
      icon: <Zap className="w-8 h-8" />,
      benefits: ['Live traffic monitoring', 'Alternative route suggestions', 'ETA accuracy']
    },
    {
      title: 'Multi-Vehicle Coordination',
      description: 'Coordinate multiple vehicles for optimal fleet performance',
      icon: <Users className="w-8 h-8" />,
      benefits: ['Fleet coordination', 'Load balancing', 'Resource optimization']
    },
    {
      title: 'Predictive Analytics',
      description: 'Forecast delays and optimize based on historical data',
      icon: <BarChart3 className="w-8 h-8" />,
      benefits: ['Delay prediction', 'Pattern recognition', 'Proactive optimization']
    }
  ]

  const benefits = [
    {
      title: 'Reduce Fuel Costs',
      description: 'Optimized routes minimize unnecessary mileage',
      icon: <Fuel className="w-6 h-6" />,
      metric: '30%'
    },
    {
      title: 'Faster Deliveries',
      description: 'Efficient routes reduce delivery times',
      icon: <Clock className="w-6 h-6" />,
      metric: '25%'
    },
    {
      title: 'Improve Customer Satisfaction',
      description: 'Accurate ETAs and faster service',
      icon: <TrendingUp className="w-6 h-6" />,
      metric: '90%'
    },
    {
      title: 'Increase Fleet Utilization',
      description: 'Better route planning maximizes vehicle efficiency',
      icon: <Route className="w-6 h-6" />,
      metric: '40%'
    }
  ]

  const optimizationTypes = [
    {
      title: 'Distance Optimization',
      description: 'Minimize total distance traveled',
      icon: <MapPin className="w-6 h-6" />
    },
    {
      title: 'Time Optimization',
      description: 'Reduce total travel time',
      icon: <Clock className="w-6 h-6" />
    },
    {
      title: 'Cost Optimization',
      description: 'Minimize fuel and operational costs',
      icon: <Fuel className="w-6 h-6" />
    },
    {
      title: 'Efficiency Optimization',
      description: 'Maximize vehicle and driver productivity',
      icon: <TrendingUp className="w-6 h-6" />
    }
  ]

  return (
    <>
      <Header />
      <main>
        {/* Hero Section */}
        <section className="pt-20 pb-16 bg-gradient-to-br from-primary-50 via-white to-primary-100">
          <div className="container-custom">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div className="space-y-8">
                <div className="inline-flex items-center space-x-2 bg-primary-100 text-primary-700 px-4 py-2 rounded-full text-sm font-medium">
                  <Star className="w-4 h-4" />
                  <span>AI-Powered Solution</span>
                </div>
                <h1 className="text-5xl md:text-6xl font-bold text-gray-900 leading-tight">
                  Route Optimization
                  <span className="block text-primary-600">That Saves Money</span>
                </h1>
                <p className="text-xl text-gray-600 leading-relaxed">
                  AI-powered route planning and optimization that reduces costs, improves efficiency, 
                  and enhances customer satisfaction through intelligent fleet management.
                </p>
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <button className="bg-primary-600 text-white px-8 py-4 rounded-lg font-semibold hover:bg-primary-700 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2">
                    <Play className="w-5 h-5" />
                    <span>Watch Demo</span>
                  </button>
                  <button className="border-2 border-primary-600 text-primary-600 px-8 py-4 rounded-lg font-semibold hover:bg-primary-50 transition-all duration-300 flex items-center space-x-2">
                    <span>Start Free Trial</span>
                    <ArrowRight className="w-5 h-5" />
                  </button>
                </div>
              </div>
              <div className="relative">
                <div className="relative overflow-hidden rounded-3xl shadow-2xl">
                  <img
                    src="https://img.freepik.com/free-photo/young-adult-traveling-using-sustainable-mobility_23-2149292780.jpg?t=st=1757534008~exp=1757537608~hmac=a2c11f5b4905f300f68dfe81a4daab9781b09a06230f69a21918ce3022237f7d&w=2000"
                    alt="Route Optimization Dashboard"
                    className="w-full h-96 object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
                  <div className="absolute bottom-6 left-6 right-6">
                    <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4">
                      <h3 className="font-semibold text-gray-900 mb-2">AI Route Planner</h3>
                      <p className="text-sm text-gray-600">Intelligent route optimization in real-time</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Key Features */}
        <section className="py-20 bg-white">
          <div className="container-custom">
            <div className="text-center max-w-4xl mx-auto mb-16">
              <h2 className="text-4xl font-bold text-gray-900 mb-6">
                Intelligent Route Planning
              </h2>
              <p className="text-xl text-gray-600">
                Advanced algorithms and real-time data for optimal fleet routing
              </p>
            </div>
            
            <div className="grid md:grid-cols-2 gap-8">
              {features.map((feature, index) => (
                <div key={index} className="bg-gray-50 rounded-2xl p-8 hover:bg-gray-100 transition-all duration-300">
                  <div className="w-16 h-16 bg-primary-100 rounded-2xl flex items-center justify-center text-primary-600 mb-6">
                    {feature.icon}
                  </div>
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">
                    {feature.title}
                  </h3>
                  <p className="text-gray-600 mb-6 leading-relaxed">
                    {feature.description}
                  </p>
                  <ul className="space-y-2">
                    {feature.benefits.map((benefit, idx) => (
                      <li key={idx} className="flex items-center space-x-3">
                        <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0" />
                        <span className="text-gray-700">{benefit}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Benefits Section */}
        <section className="py-20 bg-gray-50">
          <div className="container-custom">
            <div className="text-center max-w-4xl mx-auto mb-16">
              <h2 className="text-4xl font-bold text-gray-900 mb-6">
                Measurable Results
              </h2>
              <p className="text-xl text-gray-600">
                See immediate improvements in your fleet operations
              </p>
            </div>
            
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {benefits.map((benefit, index) => (
                <div key={index} className="text-center">
                  <div className="w-20 h-20 bg-primary-100 rounded-3xl flex items-center justify-center text-primary-600 mx-auto mb-6">
                    {benefit.icon}
                  </div>
                  <div className="text-3xl font-bold text-primary-600 mb-2">{benefit.metric}</div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-3">{benefit.title}</h3>
                  <p className="text-gray-600 text-sm leading-relaxed">{benefit.description}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Optimization Types */}
        <section className="py-20 bg-white">
          <div className="container-custom">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div className="space-y-8">
                <h2 className="text-4xl font-bold text-gray-900 mb-6">
                  Multiple Optimization Strategies
                </h2>
                <p className="text-xl text-gray-600 leading-relaxed">
                  Choose the optimization strategy that best fits your business needs, 
                  or let our AI determine the optimal approach for each situation.
                </p>
                
                <div className="grid grid-cols-2 gap-6">
                  {optimizationTypes.map((type, index) => (
                    <div key={index} className="flex items-start space-x-4">
                      <div className="w-12 h-12 bg-primary-100 rounded-xl flex items-center justify-center text-primary-600 flex-shrink-0">
                        {type.icon}
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 mb-2">{type.title}</h3>
                        <p className="text-gray-600 text-sm leading-relaxed">{type.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              
              <div className="relative">
                <div className="relative overflow-hidden rounded-3xl shadow-2xl">
                  <img
                    src="https://images.unsplash.com/photo-1574613811779-06907b5d82ce?q=80&w=2073&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                    alt="Optimization Strategies"
                    className="w-full h-96 object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Technology Section */}
        <section className="py-20 bg-gradient-to-br from-gray-900 to-gray-800 text-white">
          <div className="container-custom">
            <div className="text-center max-w-4xl mx-auto mb-16">
              <h2 className="text-4xl font-bold mb-6">
                Powered by Advanced Technology
              </h2>
              <p className="text-xl text-gray-300">
                Our route optimization engine leverages cutting-edge AI and machine learning
              </p>
            </div>
            
            <div className="grid md:grid-cols-3 gap-8">
              <div className="text-center">
                <div className="w-20 h-20 bg-primary-500/20 rounded-3xl flex items-center justify-center text-primary-400 mx-auto mb-6">
                  <Database className="w-10 h-10" />
                </div>
                <h3 className="text-xl font-semibold mb-3">Machine Learning</h3>
                <p className="text-gray-400 text-sm">AI algorithms that learn from your fleet data</p>
              </div>
              
              <div className="text-center">
                <div className="w-20 h-20 bg-primary-500/20 rounded-3xl flex items-center justify-center text-primary-400 mx-auto mb-6">
                  <Globe className="w-10 h-10" />
                </div>
                <h3 className="text-xl font-semibold mb-3">Real-time Data</h3>
                <p className="text-gray-400 text-sm">Live traffic, weather, and road condition updates</p>
              </div>
              
              <div className="text-center">
                <div className="w-20 h-20 bg-primary-500/20 rounded-3xl flex items-center justify-center text-primary-400 mx-auto mb-6">
                  <Zap className="w-10 h-10" />
                </div>
                <h3 className="text-xl font-semibold mb-3">Instant Updates</h3>
                <p className="text-gray-400 text-sm">Dynamic route adjustments as conditions change</p>
              </div>
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="py-20 bg-gradient-to-r from-primary-500 to-primary-600">
          <div className="container-custom">
            <div className="text-center max-w-4xl mx-auto text-white">
              <h2 className="text-4xl font-bold mb-6">
                Ready to Optimize Your Routes?
              </h2>
              <p className="text-xl text-white/90 mb-8">
                Join companies across Tanzania who trust FleetCo for intelligent route optimization.
              </p>
              
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-8">
                <a href="/contact" className="bg-white text-primary-600 hover:bg-gray-100 px-8 py-4 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 flex items-center space-x-2">
                  <span>Start Free Trial</span>
                  <ArrowRight className="w-5 h-5" />
                </a>
                <a href="/demo" className="border-2 border-white text-white hover:bg-white hover:text-primary-600 px-8 py-4 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 flex items-center space-x-2">
                  <Play className="w-5 h-5" />
                  <span>Schedule Demo</span>
                </a>
              </div>
              
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6 text-sm text-white/80">
                <div className="flex items-center space-x-2">
                  <Clock className="w-4 h-4" />
                  <span>14-day free trial</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Users className="w-4 h-4" />
                  <span>500+ companies trust us</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Route className="w-4 h-4" />
                  <span>AI-powered optimization</span>
                </div>
              </div>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </>
  )
}
